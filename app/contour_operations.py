...
def classify_artifacts_contextually(ct_volume, artifact_mask, metal_mask, spacing,
                                   bone_range=(500, 1500), tissue_range=(-100, 300),
                                   window_size=(7, 7, 3), artifact_type="artifact"):
    """
    Classify artifacts based on the underlying tissue they are corrupting.

    This function uses neighborhood composition analysis to determine whether each
    artifact voxel is corrupting bone or soft tissue. The classification is based
    on what HU values surround the artifact, not on the artifact's own characteristics.

    Algorithm:
        1. For each artifact voxel, examine a 7×7×3 mm neighborhood
        2. Count voxels with bone-HU (500-1500) vs soft-tissue-HU (-100 to 300)
        3. Assign scores based on neighborhood composition (primary criterion)
        4. Use distance from metal as secondary criterion (modest weight)
        5. Classify as "over bone" if bone context dominates, "over tissue" otherwise

    Args:
        ct_volume: 3D CT volume in Hounsfield Units
        artifact_mask: Binary mask of artifact voxels to classify
        metal_mask: Binary mask of metal implant regions
        spacing: Voxel spacing in mm (z, y, x)
        bone_range: HU range defining bone tissue (default: 500-1500)
        tissue_range: HU range defining soft tissue (default: -100 to 300)
        window_size: Neighborhood analysis window size in voxels (z, y, x)
        artifact_type: Descriptor for logging ("bright" or "dark")
    ...
    n_voxels = len(artifact_coords[0])
    ...
    for i in range(0, n_voxels, batch_size):
        ...
        batch_coords = (artifact_coords[0][i:end_idx],
                       artifact_coords[1][i:end_idx],
                       artifact_coords[2][i:end_idx])
        ...
        for j, (z, y, x) in enumerate(zip(*batch_coords)):
            ...
            # === CONTEXTUAL CLASSIFICATION ALGORITHM ===
            # Goal: Identify what tissue lies beneath this artifact voxel
            # Method: Score based on neighborhood composition, NOT artifact properties

            bone_context_score = 0      # Evidence for bone underneath
            tissue_context_score = 0    # Evidence for soft tissue underneath

            # PRIMARY CRITERION: Neighborhood tissue composition (weighted 5-3-1)
            # Rationale: Artifacts surrounded by bone-HU values are likely corrupting bone
            if neighborhood['bone_ratio'] > 0.6:
                bone_context_score += 5      # >60% neighbors are bone-HU
            elif neighborhood['bone_ratio'] > 0.4:
                bone_context_score += 3      # 40-60% neighbors are bone-HU
            elif neighborhood['bone_ratio'] > 0.2:
                bone_context_score += 1      # 20-40% neighbors are bone-HU

            if neighborhood['tissue_ratio'] > 0.6:
                tissue_context_score += 5    # >60% neighbors are tissue-HU
            elif neighborhood['tissue_ratio'] > 0.4:
                tissue_context_score += 3    # 40-60% neighbors are tissue-HU
            elif neighborhood['tissue_ratio'] > 0.2:
                tissue_context_score += 1    # 20-40% neighbors are tissue-HU

            # SECONDARY CRITERION: Distance from metal (modest weight +1)
            # Rationale: Distant artifacts more likely near bone structures, but not reliable alone
            if distance_mm < 5:
                pass  # Too close to metal - ambiguous (bone can be adjacent to implant)
            elif distance_mm > 30:
                bone_context_score += 1  # Far from metal suggests anatomical bone region

            # TIE-BREAKER: Use dominant ratio when scores equal
            if bone_context_score == tissue_context_score:
                if neighborhood['bone_ratio'] > neighborhood['tissue_ratio']:
                    bone_context_score += 1
                elif neighborhood['tissue_ratio'] > neighborhood['bone_ratio']:
            ...
            if bone_context_score > tissue_context_score:
                ...
                n_bone += 1
            elif tissue_context_score > bone_context_score:
...
def classify_bright_artifacts_contextually(ct_volume, bright_mask, metal_mask, spacing,
                                          ...
                                          window_size=(7, 7, 3)):
    """
    Classify bright artifacts based on underlying tissue context.

    Wrapper function for backward compatibility. Calls classify_artifacts_contextually()
    with artifact_type="bright" and returns results with legacy naming convention.

    Args:
        ct_volume: 3D CT volume in Hounsfield Units
        bright_mask: Binary mask of bright artifact voxels
        metal_mask: Binary mask of metal implant
        spacing: Voxel spacing in mm (z, y, x)
        bone_range: HU range defining bone (default: 500-1500)
        tissue_range: HU range defining soft tissue (default: -100 to 300)
        bone_distance_threshold_mm: DEPRECATED - no longer used
        bone_ratio_threshold: DEPRECATED - no longer used
        window_size: Neighborhood window size in voxels (z, y, x)
    ...
    result = classify_artifacts_contextually(
        ...
        bone_range=bone_range, tissue_range=tissue_range,
        window_size=window_size, artifact_type="bright"
    )
...
def classify_dark_artifacts_contextually(ct_volume, dark_mask, metal_mask, spacing,
                                        bone_range=(500, 1500), tissue_range=(-100, 300),
                                        window_size=(7, 7, 3)):
    """
    Classify dark artifacts based on underlying tissue context.

    Uses neighborhood composition analysis to determine whether dark artifacts
    are corrupting bone or soft tissue. Same algorithm as bright artifacts.

    Args:
        ct_volume: 3D CT volume in Hounsfield Units
        dark_mask: Binary mask of dark artifact voxels
        metal_mask: Binary mask of metal implant
        spacing: Voxel spacing in mm (z, y, x)
        bone_range: HU range defining bone (default: 500-1500)
        tissue_range: HU range defining soft tissue (default: -100 to 300)
    ...
    result = classify_artifacts_contextually(
        ...
        bone_range=bone_range, tissue_range=tissue_range,
        window_size=window_size, artifact_type="dark"
    )
...
def create_context_aware_masks(ct_volume, metal_mask, spacing, **kwargs):
    ...
    # 4. Apply contextual classification to bright artifacts
    bright_contextual = classify_bright_artifacts_contextually(
        ct_volume,
    ...
    # 5. Apply contextual classification to dark artifacts
    dark_contextual = classify_dark_artifacts_contextually(
        ct_volume,
        dark_mask,
        metal_mask,
        spacing,
...
def create_sequential_masks(ct_volume, metal_mask, spacing, discrimination_method='distance', **kwargs):
    ...
    method_map = {
        ...
        'texture': DiscriminationMethod.TEXTURE_BASED
    }
    ...
    # Perform contextual classification of dark artifacts
    dark_contextual = classify_dark_artifacts_contextually(
        ct_volume,
        dark_mask,
        metal_mask,
